#!/bin/bash
#SBATCH --job-name=e5-mistral-7b
#SBATCH --output=outputs/logs/e5-mistral-7b-%j.out
#SBATCH --error=outputs/logs/e5-mistral-7b-%j.err
...
# Default paths (same pattern as reasonir/gritlm)
QUERY_FILE="${QUERY_FILE:-./data/QUEST/test_id_added.jsonl}"
CORPUS_FILE="${CORPUS_FILE:-./data/QUEST/documents.jsonl}"
OUTPUT_FILE="${OUTPUT_FILE:-./outputs/runs/e5-mistral-7b/results_$(date +%Y%m%d_%H%M%S).jsonl}"
BATCH_SIZE="${BATCH_SIZE:-8}"
DOC_BATCH_SIZE="${DOC_BATCH_SIZE:-8}"
MAX_DOCS="${MAX_DOCS:-}"
MAX_QUERIES="${MAX_QUERIES:-}"
CACHE_DIR="${CACHE_DIR:-}"
TASK_SUFFIX="${TASK_SUFFIX:-}"
# Index file path (default under same run dir as output)
INDEX_NAME="${INDEX_NAME:-./outputs/runs/e5-mistral-7b/E5-Mistral-7B-QUEST}"
...
echo "=========================================="
echo "Starting E5-Mistral-7B-Instruct (e5-mistral-7b-instruct_main.py)"
echo "Query file: $QUERY_FILE"
echo "Corpus file: $CORPUS_FILE"
echo "Output file: $OUTPUT_FILE"
echo "Batch size: $BATCH_SIZE"
[ -n "$DOC_BATCH_SIZE" ] && echo "Doc batch size: $DOC_BATCH_SIZE"
[ -n "$MAX_DOCS" ] || [ -n "$MAX_QUERIES" ] && echo "SUBSET: max_docs=${MAX_DOCS:-ALL}, max_queries=${MAX_QUERIES:-ALL}"
...
CMD="python code/baselines/e5-mistral-7b-instruct/e5-mistral-7b-instruct_main.py \
