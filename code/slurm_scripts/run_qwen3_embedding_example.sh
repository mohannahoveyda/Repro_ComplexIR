#!/bin/bash
# Usage: bash run_qwen3_embedding_example.sh [--quest_plus]
#   --quest_plus  Use QUEST withVariants data and distinct cache/output (no overwrite of default QUEST).
# Runs Qwen3-Embedding-8B on Snellius (SLURM). Submit from repo root.
...
if [ "$QUEST_PLUS_MODE" = "1" ]; then
    ...
    export OUTPUT_FILE="./outputs/runs/qwen3-emb-8b/results_withVariants_$(date +%Y%m%d_%H%M%S).jsonl"
    export CACHE_DIR="./outputs/runs/qwen3-emb-8b/cache_withVariants"
    export INDEX_NAME="./outputs/runs/qwen3-emb-8b/Qwen3-Embedding-8B-QUEST_withVariants"
    ...
    export OUTPUT_FILE="./outputs/runs/qwen3-emb-8b/results_$(date +%Y%m%d_%H%M%S).jsonl"
fi

export BATCH_SIZE="${BATCH_SIZE:-16}"
export DOC_BATCH_SIZE="${DOC_BATCH_SIZE:-16}"
...
# export INDEX_NAME="Qwen3-Embedding-8B-QUEST"
...
# export USE_FAISS=0  # Use direct matrix multiplication (faster for smaller corpora)

# Optional: Maximum sequence length (Qwen3-Embedding supports up to 32768)
# export MAX_LENGTH=8192  # Default: 8192
...
cd "$REPO_ROOT" || exit 1
sbatch --export=ALL "$SCRIPT_DIR/qwen3_embedding_8b_gpu.sh"
