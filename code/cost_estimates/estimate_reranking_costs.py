...
def calculate_dataset_costs(
    ...
    """
    Calculate costs for a single dataset.
...
def save_to_csv(
    ...
    summary_row = {
        ...
        "instruction_tokens": str(instruction_tokens),
        "output_tokens": str(output_tokens),
        "input_cost": f"{total_input_cost:.2f}",
        "output_cost": f"{total_output_cost:.2f}",
        "total_cost": f"{total_cost:.2f}",
        "timestamp": timestamp,
    }
    filtered_rows.append(summary_row)
...
def main():
    ...
    if input_price == 0.0 and output_price == 0.0:
        print("\n⚠️  WARNING: Could not fetch pricing. Please verify:")
