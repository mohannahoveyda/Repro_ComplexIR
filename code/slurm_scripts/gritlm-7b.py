...
def main(quest_plus=False):
    ...
    with tools.benchmark(MODEL_NAME, "Embedding"):
        doc_emb = model.encode(doc_texts, instruction=gritlm_instruction(""))
        query_emb = model.encode(query, instruction=gritlm_instruction(instruction))
