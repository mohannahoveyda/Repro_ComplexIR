...
class BM25Retriever(object):
    ...
    """
        ...
        docs_seen = []
        topk_doc_ids = []
        for idx in sorted_docs_ids:
            if self.documents[idx].title not in docs_seen:
                docs_seen.append(self.documents[idx].title)
                topk_doc_ids.append(idx)
            if len(docs_seen) >= topk:
                break
