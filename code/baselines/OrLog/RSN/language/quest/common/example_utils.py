...
def read_examples(filepath):
  ...
  for example in examples:
    if example.metadata:
      # Filter metadata to only include fields that ExampleMetadata accepts
      # This allows extra fields like 'queries' and 'operators' to be preserved in JSON
      # but ignored when creating the ExampleMetadata dataclass
      metadata_fields = {
        'template', 'domain', 'fluency', 'meaning', 'naturalness',
        'relevance_ratings', 'evidence_ratings', 'attributions'
      }
      filtered_metadata = {
        k: v for k, v in example.metadata.items()
        if k in metadata_fields
      }
      example.metadata = ExampleMetadata(**filtered_metadata)
  return examples
