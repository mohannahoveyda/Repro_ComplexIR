#!/bin/bash
#SBATCH --job-name=promptriever-7b
#SBATCH --output=outputs/logs/promptriever-7b-%j.out
#SBATCH --error=outputs/logs/promptriever-7b-%j.err
...
python -c 'import torch; print("PyTorch:", torch.__version__, "| CUDA available:", torch.cuda.is_available())' || { echo "ERROR: No module torch in env $CONDA_ENV_NAME."; exit 1; }
...
OUTPUT_FILE="${OUTPUT_FILE:-./outputs/runs/promptriever-7b/results_$(date +%Y%m%d_%H%M%S).jsonl}"
...
echo "=========================================="
echo "Starting Promptriever-7B inference"
...
CMD="python code/baselines/promptretriever/promptriever-7b.py \
