...
def read_documents(filepath, limit=None):
  ...
  # Filter documents to only include fields that Document dataclass accepts
  # This allows extra fields like 'label' and 'idx' to be preserved in JSON
  # but ignored when creating Document objects
  filtered_documents = [
    {k: v for k, v in document.items() if k in ('title', 'text')}
    for document in documents_json
  ]
  return [Document(**document) for document in filtered_documents]
